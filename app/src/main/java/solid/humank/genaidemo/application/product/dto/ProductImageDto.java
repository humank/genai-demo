...
public record ProductImageDto(
        @Schema(description = "圖片唯一識別碼", example = "img-001", required = true)
                @NotBlank(message = "圖片ID不能為空")
                String id,
        @Schema(
                        description = "圖片URL",
                ...
                @Pattern(
                        ...
                        message = "圖片URL格式不正確，必須是有效的HTTP(S)連結且以圖片格式結尾")
                String url,
        @Schema(description = "圖片替代文字", example = "iPhone 15 Pro 太空黑色正面圖", maxLength = 200)
                @Size(max = 200, message = "圖片替代文字不能超過200個字元")
                String altText,
        @Schema(description = "圖片標題", example = "iPhone 15 Pro - 正面視圖", maxLength = 100)
                @Size(max = 100, message = "圖片標題不能超過100個字元")
                String title,
        @Schema(
                        description = "圖片類型",
                        allowableValues = {"PRIMARY", "GALLERY", "THUMBNAIL", "DETAIL"},
                        example = "PRIMARY",
                        required = true)
                @NotBlank(message = "圖片類型不能為空")
                String type,
        @Schema(description = "圖片排序順序", example = "1", minimum = "0", required = true)
                int sortOrder,
