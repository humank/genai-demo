...
/** 統計應用服務 - 重構為使用 JPA 符合六角形架構的應用層服務，依賴於領域層的儲存庫接口 */
@Service
public class StatsApplicationService {
    ...
    /** 獲取數據庫統計信息 使用 JPA Repository 方法替代原生 SQL */
    ...
    public OrderStatusStatsDto getOrderStatusStats() {
        try {
            ...
            statusCounts.forEach(
                    (status, count) -> integerStatusCounts.put(status, count.intValue()));

            return new OrderStatusStatsDto(integerStatusCounts, "success", null);

        } catch (Exception e) {
            return new OrderStatusStatsDto(
                    new HashMap<>(), "error", "獲取訂單狀態統計時發生錯誤: " + e.getMessage());
        }
    }

    /** 獲取支付方式分布 使用 JPA 查詢方法替代原生 SQL */
    public PaymentMethodStatsDto getPaymentMethodStats() {
        try {
            ...
            methodCounts.forEach(
                    (method, count) -> integerMethodCounts.put(method, count.intValue()));

            return new PaymentMethodStatsDto(integerMethodCounts, "success", null);

        } catch (Exception e) {
            return new PaymentMethodStatsDto(
                    new HashMap<>(), "error", "獲取支付方式統計時發生錯誤: " + e.getMessage());
        }
    }
}
