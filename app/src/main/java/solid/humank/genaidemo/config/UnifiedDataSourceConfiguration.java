 ...
 *
 * This configuration consolidates all database configuration logic into a
 * single class,
 * handling different environments (local, test, staging, production) and
 * credential sources
 * (local configuration, AWS Secrets Manager) based on active profiles and
 * properties.
 */
@Configuration
public class UnifiedDataSourceConfiguration {
    ...
    private DataSource createSecretsManagerDataSource() {
        ...
        try {
            ...
            logger.error("Failed to configure DataSource with Secrets Manager", e);
            throw new DatabaseConfigurationException(
                    "Failed to configure database with Secrets Manager: " + e.getMessage(), e);
        }
    }
