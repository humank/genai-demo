...
public class DomainEventMonitor {
    ...
    public Object monitorEventHandler(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        try {
            ...
            LOGGER.info(() -> String.format("事件處理完成: %s, 處理器: %s, 耗時: %d ms", 
                                          eventType, handlerName, duration));
            
            return result;
        } catch (Throwable e) {
            long duration = System.currentTimeMillis() - startTime;
            LOGGER.log(Level.SEVERE, String.format("事件處理失敗: %s, 處理器: %s, 耗時: %d ms, 錯誤: %s", 
                                                eventType, handlerName, duration, e.getMessage()), e);
            throw e;
        }
    }
}