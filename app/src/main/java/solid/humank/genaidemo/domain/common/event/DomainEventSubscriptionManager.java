 ...
 */
@Component
public class DomainEventSubscriptionManager implements BeanPostProcessor, ApplicationListener<ApplicationEvent> {
    ...
    private void registerSubscriber(Object bean, Method method, EventSubscriber annotation) {
        ...
        // 檢查方法參數是否與事件類型匹配
        if (method.getParameterCount() != 1 || !eventType.isAssignableFrom(method.getParameterTypes()[0])) {
            LOGGER.warning(() -> String.format(
                "Method %s in %s has @EventSubscriber annotation but parameter types don't match event type %s",
                method.getName(), bean.getClass().getName(), eventType.getName()));
            return;
        }
    ...
    private void notifySubscribers(Map<Object, Method> subscribers, DomainEvent event) {
        subscribers.forEach((bean, method) -> {
            try {
