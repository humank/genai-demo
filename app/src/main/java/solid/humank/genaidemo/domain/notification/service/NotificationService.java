...
public class NotificationService {
     ...
     * @return 通知ID
     */
    public NotificationId sendOrderCreationNotification(
            String customerId,
            OrderId orderId,
        ...
        String content =
                String.format("您的訂單 %s 已於 %s 創建成功。", orderId.toString(), orderCreationTime);

        Notification notification =
                new Notification(
                        customerId, NotificationType.ORDER_CREATED, subject, content, channels);
     ...
     * @return 通知ID
     */
    public NotificationId sendOrderConfirmationNotification(
            String customerId,
            OrderId orderId,
     ...
     * @return 通知ID
     */
    public NotificationId sendPaymentFailureNotification(
            String customerId,
            OrderId orderId,
            String failureReason,
            String retryPaymentLink,
            List<NotificationChannel> channels) {
        ...
        Notification notification =
                new Notification(
                        customerId, NotificationType.PAYMENT_FAILED, subject, content, channels);
     ...
     * @return 通知ID
     */
    public NotificationId sendInsufficientInventoryNotification(
            String customerId,
            OrderId orderId,
            List<String> outOfStockProducts,
            Map<String, String> alternativeProducts,
            List<NotificationChannel> channels) {
     ...
     * @return 通知ID
     */
    public NotificationId sendDeliveryStatusUpdateNotification(
            String customerId,
            OrderId orderId,
     ...
     * @return 通知ID
     */
    public NotificationId sendOrderCompletionNotification(
            String customerId,
            OrderId orderId,
            String ratingLink,
     ...
     * @return 通知ID
     */
    public NotificationId sendOrderCancellationNotification(
            String customerId,
            OrderId orderId,
        ...
        String content =
                String.format("您的訂單 %s 已取消。\n取消原因：%s", orderId.toString(), cancellationReason);

        Notification notification =
                new Notification(
                        customerId, NotificationType.ORDER_CANCELLED, subject, content, channels);
    ...
    private void scheduleRetry(Notification notification) {
        if (notification.getRetryCount() < 3) {
            ...
            LocalDateTime retryTime =
                    LocalDateTime.now().plusHours(1 << notification.getRetryCount());
            notification.retry();
     ...
     * @return 是否更新成功
     */
    public boolean updateCustomerNotificationPreferences(
            String customerId,
