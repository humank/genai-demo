...
import solid.humank.genaidemo.domain.inventory.service.InventoryService;
...
import solid.humank.genaidemo.domain.payment.model.service.PaymentService;

/** 訂單工作流服務 負責處理訂單工作流的創建、狀態轉換和完成 */
@DomainService
public class OrderWorkflowService {
    ...
    public boolean processPayment(
            WorkflowId workflowId, String paymentMethod, Map<String, Object> paymentDetails) {
        Optional<OrderWorkflow> optionalWorkflow = orderWorkflowRepository.findById(workflowId);
        if (optionalWorkflow.isEmpty()) {
            return false;
        }
        ...
        // 簡化實作：使用現有的驗證方法
        // 簡化實作：使用預設金額
        solid.humank.genaidemo.domain.common.valueobject.Money defaultAmount = solid.humank.genaidemo.domain.common.valueobject.Money
                .twd(1000);
        boolean isSuccessful = paymentService.isValidPaymentRequest(
                workflow.getOrderId().getId(),
                defaultAmount,
                solid.humank.genaidemo.domain.payment.model.valueobject.PaymentMethod
    ...
    private void sendOrderConfirmationNotification(OrderWorkflow workflow) {
        ...
        notificationService.sendOrderConfirmationNotification(
                customerId, workflow.getOrderId(), orderDetails, estimatedDeliveryTime, channels);
    }
    ...
    private void sendPaymentFailureNotification(OrderWorkflow workflow, String failureReason) {
        ...
        notificationService.sendPaymentFailureNotification(
                customerId, workflow.getOrderId(), failureReason, retryPaymentLink, channels);
    }
    ...
    private void sendOrderCompletionNotification(OrderWorkflow workflow) {
        ...
        notificationService.sendOrderCompletionNotification(
                customerId, workflow.getOrderId(), ratingLink, recommendations, channels);
    }
    ...
    private void sendOrderCancellationNotification(OrderWorkflow workflow) {
        ...
        notificationService.sendOrderCancellationNotification(
                customerId, workflow.getOrderId(), cancellationReason, channels);
    }
