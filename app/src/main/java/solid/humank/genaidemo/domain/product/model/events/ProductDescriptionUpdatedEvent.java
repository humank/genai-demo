package solid.humank.genaidemo.domain.product.model.events;

import java.time.LocalDateTime;
import java.util.UUID;

import solid.humank.genaidemo.domain.common.event.DomainEvent;
import solid.humank.genaidemo.domain.product.model.valueobject.ProductDescription;
import solid.humank.genaidemo.domain.product.model.valueobject.ProductId;

/**
 * 商品描述更新事件 當商品描述發生變更時發布此事件
 * 使用 record 實作，自動獲得不可變性和基礎功能
 */
public record ProductDescriptionUpdatedEvent(
        ProductId productId,
        ProductDescription oldDescription,
        ProductDescription newDescription,
        UUID eventId,
        LocalDateTime occurredOn) implements DomainEvent {

    /**
     * 工廠方法，自動設定 eventId 和 occurredOn
     */
    public static ProductDescriptionUpdatedEvent create(
            ProductId productId,
            ProductDescription oldDescription,
            ProductDescription newDescription) {
        DomainEvent.EventMetadata metadata = DomainEvent.createEventMetadata();
        return new ProductDescriptionUpdatedEvent(productId, oldDescription, newDescription,
                metadata.eventId(), metadata.occurredOn());
    }

    @Override
    public UUID getEventId() {
        return eventId;
    }

    @Override
    public LocalDateTime getOccurredOn() {
        return occurredOn;
    }

    @Override
    public String getEventType() {
        return DomainEvent.getEventTypeFromClass(this.getClass());
    }

    @Override
    public String getAggregateId() {
        return productId.getId();
    }
}
