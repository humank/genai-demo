...
/** 捆綁銷售服務 */
@DomainService(name = "BundleService", description = "捆綁銷售服務，處理產品捆綁銷售的業務邏輯和折扣計算", boundedContext = "Product")
    ...
    public Money applyBundleDiscount(Order order, Bundle bundle) {
        if (bundle.isFixedBundle()) {
            ...
            return order.getTotalAmount()
                    .subtract(bundle.getDiscount().calculateDiscount(order.getTotalAmount()));
        }
    }
