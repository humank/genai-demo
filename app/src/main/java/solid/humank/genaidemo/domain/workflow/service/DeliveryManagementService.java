...
/** 配送管理服務 負責處理配送的創建、狀態轉換和完成 */
@DomainService(description = "配送管理服務，負責處理配送的創建、狀態轉換和完成")
public class DeliveryManagementService implements DeliveryService {
    ...
    public boolean allocateDeliveryResources(OrderId orderId) {
        ...
        // 發送配送狀態更新通知
        sendDeliveryStatusUpdateNotification(
                orderId, DeliveryStatus.IN_TRANSIT, delivery.getEstimatedDeliveryTime());
    ...
    public boolean updateEstimatedDeliveryTime(
            UUID deliveryId, LocalDateTime estimatedDeliveryTime) {
    ...
    private void sendDeliveryStatusUpdateNotification(
            OrderId orderId, DeliveryStatus status, LocalDateTime estimatedDeliveryTime) {
        ...
        notificationService.sendDeliveryStatusUpdateNotification(
                customerId, orderId, status.name(), estimatedDeliveryTime, trackingLink, channels);
    }
