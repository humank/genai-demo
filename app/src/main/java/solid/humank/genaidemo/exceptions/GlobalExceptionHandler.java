...
public class GlobalExceptionHandler {
        ...
        @ApiResponse(responseCode = "422", description = "業務規則違反", content = @Content(mediaType = "application/json", schema = @Schema(implementation = StandardErrorResponse.class)))
        @Hidden
        ...
        @ApiResponse(responseCode = "400", description = "請求參數驗證失敗", content = @Content(mediaType = "application/json", schema = @Schema(implementation = StandardErrorResponse.class)))
        @Hidden
        ...
        public StandardErrorResponse handleMethodArgumentNotValidException(
                ...
                List<StandardErrorResponse.FieldError> details = ex.getBindingResult().getFieldErrors().stream()
                                .map(
                                                fieldError -> new StandardErrorResponse.FieldError(
                                                                fieldError.getField(),
        ...
        @ResponseStatus(HttpStatus.BAD_REQUEST)
        @ApiResponse(responseCode = "400", description = "約束驗證失敗", content = @Content(mediaType = "application/json", schema = @Schema(implementation = StandardErrorResponse.class)))
        @Hidden
        public StandardErrorResponse handleConstraintViolationException(
                ...
                List<StandardErrorResponse.FieldError> details = ex.getConstraintViolations().stream()
                                .map(
                                                violation -> new StandardErrorResponse.FieldError(
        ...
        @ResponseStatus(HttpStatus.BAD_REQUEST)
        @ApiResponse(responseCode = "400", description = "參數類型不匹配", content = @Content(mediaType = "application/json", schema = @Schema(implementation = StandardErrorResponse.class)))
        @Hidden
        ...
        @ApiResponse(responseCode = "404", description = "請求的資源不存在", content = @Content(mediaType = "application/json", schema = @Schema(implementation = StandardErrorResponse.class)))
        @Hidden
        ...
        @ApiResponse(responseCode = "500", description = "事件處理失敗", content = @Content(mediaType = "application/json", schema = @Schema(implementation = StandardErrorResponse.class)))
        @Hidden
        public StandardErrorResponse handleEventProcessingException(
                        EventProcessingException ex, HttpServletRequest request) {

                String traceId = generateTraceId();
                logger.error(
                                "Event processing exception occurred - TraceId: {}, Path: {}, Event: {}, Handler: {}, Attempts: {}",
                                traceId,
                                request.getRequestURI(),
                                ex.getEvent().getEventType(),
                                ex.getEventHandlerName(),
                                ex.getAttemptCount(),
                                ex);

                String message = String.format("事件處理失敗: %s (處理器: %s, 嘗試次數: %d)",
                                ex.getEvent().getEventType(),
                                ex.getEventHandlerName(),
                                ex.getAttemptCount());
        ...
        @ApiResponse(responseCode = "408", description = "事件處理超時", content = @Content(mediaType = "application/json", schema = @Schema(implementation = StandardErrorResponse.class)))
        @Hidden
        public StandardErrorResponse handleEventProcessingTimeoutException(
                        EventProcessingTimeoutException ex, HttpServletRequest request) {

                String traceId = generateTraceId();
                logger.error(
                                "Event processing timeout occurred - TraceId: {}, Path: {}, Event: {}, Handler: {}, Timeout: {}ms, Actual: {}ms",
                                traceId,
                                request.getRequestURI(),
                                ex.getEvent().getEventType(),
                                ex.getEventHandlerName(),
                                ex.getTimeoutMillis(),
                                ex.getActualProcessingTime(),
                                ex);

                String message = String.format("事件處理超時: %s (處理器: %s, 超時時間: %dms, 實際時間: %dms)",
                                ex.getEvent().getEventType(),
                                ex.getEventHandlerName(),
                                ex.getTimeoutMillis(),
                                ex.getActualProcessingTime());
        ...
        @ApiResponse(responseCode = "409", description = "事件順序錯誤", content = @Content(mediaType = "application/json", schema = @Schema(implementation = StandardErrorResponse.class)))
        @Hidden
        public StandardErrorResponse handleEventSequenceException(
                        EventSequenceException ex, HttpServletRequest request) {

                String traceId = generateTraceId();
                logger.error(
                                "Event sequence exception occurred - TraceId: {}, Path: {}, Event: {}, Handler: {}, Aggregate: {}, Expected: {}, Actual: {}",
                                traceId,
                                request.getRequestURI(),
                                ex.getEvent().getEventType(),
                                ex.getEventHandlerName(),
                                ex.getAggregateId(),
                                ex.getExpectedSequence(),
                                ex.getActualSequence(),
                                ex);

                String message = String.format("事件順序錯誤: %s (聚合根: %s, 期望序列: %d, 實際序列: %d)",
                                ex.getEvent().getEventType(),
                                ex.getAggregateId(),
                                ex.getExpectedSequence(),
                                ex.getActualSequence());
                ...
                return new StandardErrorResponse(
                                ErrorCode.BUSINESS_RULE_VIOLATION, message, request.getRequestURI(), null, traceId);
        }
