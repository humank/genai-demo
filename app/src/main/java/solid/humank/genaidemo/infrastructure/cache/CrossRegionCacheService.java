...
public class CrossRegionCacheService {
    ...
    public <T> Optional<T> get(String key, Class<T> valueType, Supplier<Optional<T>> dataSource, Duration ttl) {
        ...
        try {
            ...
            if (isInvalidated(normalizedKey)) {
                ...
                return loadAndCache(normalizedKey, valueType, dataSource, ttl, startTime);
            }
            ...
            if (cachedJson != null) {
                try {
                    T cachedValue = objectMapper.readValue(cachedJson, valueType);
                    ...
                    logger.warn("Failed to deserialize cached value for key: {}", normalizedKey, e);
                    cacheMetrics.recordCacheDeserializationError(normalizedKey);
                    // Continue to load from data source
                }
            }
    ...
    public void invalidate(String key) {
        String normalizedKey = normalizeKey(key);
        String lockKey = "cache-invalidation:" + normalizedKey;
    ...
    public void invalidatePattern(String keyPattern) {
        ...
        distributedLockService.executeWithLock(lockKey, () -> {
            ...
            try {
                ...
                throw new CrossRegionCacheException("Failed to invalidate cache pattern", e);
            }
        });
    }
     ...
     */
    private <T> Optional<T> loadAndCache(String normalizedKey, Class<T> valueType, 
