 ...
 * Manages database configuration selection and validation based on active profiles
 */
@Configuration
    ...
    private DatabaseConfiguration selectDatabaseConfiguration() {
        ...
        for (String profile : activeProfiles) {
            if ("test".equals(profile)) {
                log.info("Test profile detected, using H2 database configuration");
                return findConfigurationByType("h2");
            }
        }
        
        // Default to development if no recognized profile is found
        log.warn("No recognized database profile found in {}, defaulting to H2", java.util.Arrays.toString(activeProfiles));
        return findConfigurationByType("h2");
    }
