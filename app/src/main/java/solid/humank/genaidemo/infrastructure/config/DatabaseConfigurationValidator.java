...
public class DatabaseConfigurationValidator {
    ...
    private void validateRequiredTables(Connection connection) throws SQLException {
        ...
        for (String tableName : requiredTables) {
            try (var resultSet = metaData.getTables(null, null, tableName.toUpperCase(), new String[]{"TABLE"})) {
                if (!resultSet.next()) {
                    ...
                    try (var resultSet2 = metaData.getTables(null, null, tableName.toLowerCase(), new String[]{"TABLE"})) {
                        if (!resultSet2.next()) {
        ...
        if (!missingTables.isEmpty()) {
            log.warn("Some expected tables are missing: {}. This might be normal if migrations haven't run yet.", missingTables);
        } else {
