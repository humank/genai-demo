...
@Component
public class OrderMapper implements DomainMapper<Order, JpaOrderEntity> {
         ...
         */
        @Override
        public Order toDomainModel(JpaOrderEntity jpaEntity) {
                try {
                        // 創建必要的值對象
                        ...
                        // 轉換訂單項
                        List<OrderItem> orderItems = jpaEntity.getItems().stream()
                                        .map(
                                                        item -> {
                        ...
                        Money effectiveAmount = Money.of(
        ...
        /** 備用方法：當反射方法失敗時使用 */
        private Order createOrderAlternative(JpaOrderEntity jpaEntity) {
                // 創建基本訂單
                Order order = new Order(
                                OrderId.of(jpaEntity.getId()),
                                CustomerId.of(jpaEntity.getCustomerId()),
                                jpaEntity.getShippingAddress());
