...
/** 配送儲存庫適配器 用於適配新舊兩個配送儲存庫 */
@Repository
public class DeliveryRepositoryAdapter implements DeliveryRepository {

    private final solid.humank.genaidemo.domain.workflow.repository.DeliveryRepository
            workflowDeliveryRepository;

    public DeliveryRepositoryAdapter(
            solid.humank.genaidemo.domain.workflow.repository.DeliveryRepository
                    workflowDeliveryRepository) {
    ...
    public List<Delivery> findAll() {
        ...
        return workflowDeliveryRepository.findAll().stream()
    ...
    public Optional<Delivery> findById(DeliveryId id) {
        // 使用UUID查詢舊的Delivery
        return workflowDeliveryRepository.findById(id.getId()).map(this::convertToNewDelivery);
    }

    @Override
    public Optional<Delivery> findByUUID(UUID id) {
        // 直接使用UUID查詢舊的Delivery
        return workflowDeliveryRepository.findById(id).map(this::convertToNewDelivery);
    }

    @Override
    public Optional<Delivery> findByOrderId(OrderId orderId) {
        // 使用OrderId查詢舊的Delivery
        return workflowDeliveryRepository.findByOrderId(orderId).map(this::convertToNewDelivery);
    }
    ...
    public List<Delivery> findByStatus(DeliveryStatus status) {
        ...
        return workflowDeliveryRepository.findByStatus(oldStatus).stream()
                .map(this::convertToNewDelivery)
                .collect(Collectors.toList());
    }
    ...
    public List<Delivery> findFailedDeliveries() {
        // 使用舊的儲存庫查詢失敗的配送
        return workflowDeliveryRepository.findFailedDeliveries().stream()
                .map(this::convertToNewDelivery)
                .collect(Collectors.toList());
    }
    ...
    public List<Delivery> findDeliveriesForRedelivery() {
        // 使用舊的儲存庫查詢需要重新配送的配送
        return workflowDeliveryRepository.findDeliveriesForRedelivery().stream()
                .map(this::convertToNewDelivery)
                .collect(Collectors.toList());
    }
    ...
    public List<Delivery> findByDeliveryPersonId(String deliveryPersonId) {
        // 使用舊的儲存庫查詢指定配送人員的配送
        return workflowDeliveryRepository.findByDeliveryPersonId(deliveryPersonId).stream()
                .map(this::convertToNewDelivery)
                .collect(Collectors.toList());
    }

    /** 將新的Delivery轉換為舊的Delivery */
    private solid.humank.genaidemo.domain.workflow.model.aggregate.Delivery convertToOldDelivery(
            Delivery newDelivery) {
        // 在實際實現中，這裡需要進行詳細的轉換
        // 這裡只是一個簡單的示例
        return new solid.humank.genaidemo.domain.workflow.model.aggregate.Delivery(
                newDelivery.getOrderId(), newDelivery.getShippingAddress());
    }

    /** 將舊的Delivery轉換為新的Delivery */
    private Delivery convertToNewDelivery(
            solid.humank.genaidemo.domain.workflow.model.aggregate.Delivery oldDelivery) {
        // 在實際實現中，這裡需要進行詳細的轉換
        // 這裡只是一個簡單的示例
        return new Delivery(
    ...
    /** 將新的DeliveryStatus轉換為舊的DeliveryStatus */
    private solid.humank.genaidemo.domain.workflow.model.valueobject.DeliveryStatus
            convertToOldDeliveryStatus(DeliveryStatus newStatus) {
        // 在實際實現中，這裡需要進行詳細的轉換
        // 這裡只是一個簡單的示例
        return switch (newStatus) {
            case PENDING_SHIPMENT ->
                    solid.humank.genaidemo.domain.workflow.model.valueobject.DeliveryStatus
                            .PENDING_SHIPMENT;
            case IN_TRANSIT ->
                    solid.humank.genaidemo.domain.workflow.model.valueobject.DeliveryStatus
                            .IN_TRANSIT;
            case DELIVERED ->
                    solid.humank.genaidemo.domain.workflow.model.valueobject.DeliveryStatus
                            .DELIVERED;
            case DELAYED ->
                    solid.humank.genaidemo.domain.workflow.model.valueobject.DeliveryStatus.DELAYED;
            case DELIVERY_FAILED ->
                    solid.humank.genaidemo.domain.workflow.model.valueobject.DeliveryStatus
                            .DELIVERY_FAILED;
            case REFUSED ->
                    solid.humank.genaidemo.domain.workflow.model.valueobject.DeliveryStatus.REFUSED;
            case CANCELLED ->
