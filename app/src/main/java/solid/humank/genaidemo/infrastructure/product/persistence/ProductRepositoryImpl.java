...
public class ProductRepositoryImpl implements ProductRepository {
    ...
    public Optional<Product> findById(ProductId productId) {
        return jpaRepository.findByProductIdAndStatus(productId.getId(), ACTIVE_STATUS)
                .map(mapper::toDomain);
    }
    
    @Override
    public List<Product> findByCategory(ProductCategory category) {
        return jpaRepository.findByCategoryAndStatus(category.getName(), ACTIVE_STATUS)
                .stream()
                .map(mapper::toDomain)
    ...
    public List<Product> findByName(String name) {
        return jpaRepository.findByNameContainingIgnoreCaseAndStatus(name, ACTIVE_STATUS)
                .stream()
                .map(mapper::toDomain)
                .collect(Collectors.toList());
    }
    ...
    public void deleteById(ProductId productId) {
        jpaRepository.findByProductIdAndStatus(productId.getId(), ACTIVE_STATUS)
                .ifPresent(entity -> {
    ...
    public List<Product> findAll() {
        return jpaRepository.findByStatus(ACTIVE_STATUS)
                .stream()
                .map(mapper::toDomain)
                .collect(Collectors.toList());
    }
    ...
    public Page<Product> findProducts(int page, int size) {
        ...
        return jpaRepository.findByStatus(ACTIVE_STATUS, pageable)
                .map(mapper::toDomain);
    }
}