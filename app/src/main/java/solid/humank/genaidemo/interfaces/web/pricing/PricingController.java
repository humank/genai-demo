...
public class PricingController {
     ...
     */
    @PostMapping("/rules")
    public ResponseEntity<PricingRuleDto> createPricingRule(@RequestBody CreatePricingRuleRequest request) {
        ...
        CreatePricingRuleCommand command = new CreatePricingRuleCommand(
    ...
    public ResponseEntity<PricingRuleDto> updateCommissionRate(
            ...
            @RequestBody UpdateCommissionRateRequest request) {
        
        UpdateCommissionRateCommand command = new UpdateCommissionRateCommand(
                priceId,
                request.getNormalRate(),
                request.getEventRate()
        );
    ...
    public ResponseEntity<List<PricingRuleDto>> getPricingRulesForProduct(@PathVariable String productId) {
    ...
    public ResponseEntity<List<PricingRuleDto>> getPricingRulesByCategory(@PathVariable solid.humank.genaidemo.interfaces.web.pricing.dto.ProductCategoryDto category) {
        // 將介面層的 ProductCategoryDto 轉換為應用層的 ProductCategoryDto
        ProductCategoryDto categoryDto = convertToAppProductCategory(category);
        List<PricingRuleDto> rules = pricingApplicationService.getPricingRulesByCategory(categoryDto);
        return ResponseEntity.ok(rules);
    }
