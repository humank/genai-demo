...
/**
 * 字符串模式匹配工具類
 * 展示 Java 21 的 Pattern Matching 功能
 */
public class StringPatternMatcher {
    ...
    public static String checkInputType(Object input) {
        return switch (input) {
            ...
            case String s when isValidPhoneNumber(s) -> STR."輸入為有效的電話號碼: \{s}";
            case String s when isValidIdNumber(s) -> STR."輸入為有效的身份證號碼: \{s}";
            case String s when isValidUrl(s) -> STR."輸入為有效的 URL: \{s}";
            ...
            case java.util.Map<?, ?> map -> STR."輸入為映射，大小為: \{map.size()}";
    ...
    public static String parseInput(Object input) {
        if (input instanceof String text) {
            if (text.contains("@")) {
                return STR."可能是電子郵件: \{text}";
            } else if (text.matches("\\d+")) {
                return STR."可能是數字字符串: \{text}";
            } else {
                return STR."普通文本: \{text}";
            }
            ...
            return STR."不支持的類型: \{input != null ? input.getClass().getSimpleName() : "null"}";
        }
    }
}