...
public class FlashSaleStepDefinitions {
    ...
    @Given("a product {string} is on flash sale from {int}:{int} to {int}:{int} \\(GMT+{int}) at ${int} instead of ${int}")
    public void a_product_is_on_flash_sale_from_to_gmt_at_$_instead_of_$(String productName, Integer startHour, 
    ...
    public void the_customer_checks_out_at_gmt(Integer hour, Integer minute, Integer timezone) {
        ...
        // 檢查是否在特價時間內
        boolean isWithinSaleTime = checkoutTime.isAfter(flashSaleRule.getStartTime()) && 
    ...
    @Given("a product {string} has a limited quantity of {int} units at a special price of ${int} instead of ${int}")
