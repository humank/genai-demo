...
public class EnhancedAssertions {
    ...
    public static void assertOrderStatus(Order order, OrderStatus expectedStatus) {
        ...
        assertEquals(
                ...
                order.getStatus(),
                String.format(
                        "Order %s should have status %s but was %s",
    ...
    public static void assertOrderTotalAmount(Order order, BigDecimal expectedAmount) {
        ...
        assertEquals(
                0,
                expectedAmount.compareTo(order.getTotalAmount().getAmount()),
                String.format(
                        "Order %s should have total amount %s but was %s",
    ...
    public static void assertOrderEffectiveAmount(Order order, BigDecimal expectedAmount) {
        ...
        assertEquals(
                0,
                expectedAmount.compareTo(order.getEffectiveAmount().getAmount()),
                String.format(
                        "Order %s should have effective amount %s but was %s",
    ...
    public static void assertOrderItemCount(Order order, int expectedCount) {
        ...
        assertEquals(
                ...
                order.getItems().size(),
                String.format(
                        "Order %s should have %d items but had %d",
    ...
    public static void assertOrderCustomerId(Order order, String expectedCustomerId) {
        ...
        assertEquals(
                expectedCustomerId,
                order.getCustomerIdAsString(),
                String.format(
                        "Order %s should have customer ID %s but was %s",
    ...
    public static void assertOrderDiscountAmount(Order order, BigDecimal expectedDiscount) {
        ...
        BigDecimal actualDiscount =
                order.getTotalAmount().getAmount().subtract(order.getEffectiveAmount().getAmount());
        assertEquals(
                0,
                expectedDiscount.compareTo(actualDiscount),
                String.format(
                        "Order %s should have discount amount %s but was %s",
    ...
    public static void assertMoneyAmount(Money money, BigDecimal expectedAmount) {
        ...
        assertEquals(
                0,
                expectedAmount.compareTo(money.getAmount()),
                String.format(
                        "Money should have amount %s but was %s",
    ...
    public static void assertExceptionContainsMessage(Exception exception, String expectedMessage) {
        ...
        assertTrue(
                ...
                String.format(
                        "Exception message should contain '%s' but was '%s'",
                        expectedMessage, exception.getMessage()));
    }

    /** 斷言異常類型和訊息 */
    public static void assertExceptionTypeAndMessage(
            Exception exception, Class<? extends Exception> expectedType, String expectedMessage) {
        assertNotNull(exception, "Exception should not be null");
        assertTrue(
                expectedType.isInstance(exception),
                String.format(
                        "Exception should be of type %s but was %s",
                        expectedType.getSimpleName(), exception.getClass().getSimpleName()));
        assertExceptionContainsMessage(exception, expectedMessage);
    }

    /** 斷言集合不為空且包含指定數量的元素 */
    public static <T> void assertCollectionSizeAndNotEmpty(
            java.util.Collection<T> collection, int expectedSize) {
        ...
        assertEquals(
                ...
                collection.size(),
                String.format(
                        "Collection should have size %d but was %d",
