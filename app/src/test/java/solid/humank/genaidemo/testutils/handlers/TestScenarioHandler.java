...
/**
 * 測試場景處理器
 * 用於處理BDD步驟定義中的複雜邏輯，避免在步驟定義中使用條件語句
 */
public class TestScenarioHandler {
    ...
    public void handleAddItemScenario(Order order, String productName, int quantity, int price, 
        ...
        try {
            if (isExpensiveProductScenario(productName, price)) {
                exceptionHandler.accept(new IllegalArgumentException(TestConstants.ErrorMessages.ORDER_AMOUNT_EXCEEDED));
                return;
            }
     ...
     * 處理支付場景
     */
    public PaymentResult handlePaymentScenario(String paymentMethod, Map<String, Object> paymentDetails, Money orderTotal) {
     ...
     * 處理庫存場景
     */
    public void handleInventoryScenario(String action, String productName, 
    ...
    private PaymentResult handleCashbackScenario(Map<String, Object> paymentDetails, Money orderTotal) {
        int percentage = (int) paymentDetails.get("cashbackPercentage");
        int maxCashback = paymentDetails.containsKey("maxCashback") ? 
            (int) paymentDetails.get("maxCashback") : Integer.MAX_VALUE;
    ...
    private PaymentResult handleInstantDiscountScenario(Map<String, Object> paymentDetails, Money orderTotal) {
        int discount = (int) paymentDetails.get("instantDiscount");
        int minOrderAmount = paymentDetails.containsKey("minOrderAmount") ? 
            (int) paymentDetails.get("minOrderAmount") : 0;
