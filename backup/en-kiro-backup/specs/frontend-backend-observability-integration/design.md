...
本設計文件概述了 Angular 消費者前端與 Spring Boot 後端之間Observability能力的全面整合，為電商系統創建統一的全棧Monitoring和分析平台。

設計重點在於建立從前端用戶互動到後端業務Metrics的無縫數據流，實現對用戶行為、系統效能和業務成果的完整可見性。解決方案建立在現有強大的後端Observability基礎設施之上，同時添加全面的前端Monitoring能力。

### Design

1. **端到端可追溯性**: 每個用戶請求都可以從前端Tracing到Repository
2. **即時分析**: 業務Metrics可近即時提供以供決策制定
3. **效能優先**: Monitoring開銷不得影響用戶體驗
...
graph TB
    ...
    subgraph "Domain Layer (現有 DDD 系統)"
        H[Domain Event發布器]
        I[Aggregate Root]
        J[Domain Event處理器]
    end
    
    subgraph "Environment差異化事件處理"
        K{Profile 檢查}
        L[記憶體事件處理器<br/>dev/test]
        M[MSK 事件發布器<br/>production]
    end
    
    subgraph "生產Environment - MSK"
        N[Amazon MSK 叢集]
        O[Kafka Topics]
        P[DLQ 處理]
    end
    
    subgraph "Monitoring與分析"
        Q[即時分析引擎]
        R[業務MetricsAggregate]
        S[WebSocket 通知]
    end
...
1. **前端收集**: 透過Observability SDK 收集用戶互動和效能Metrics
2. **批次處理**: 數據在本地緩衝並以優化批次發送到後端
3. **Domain Event轉換**: 後端將前端事件轉換為Domain Event
4. **記憶體處理**: 事件在記憶體中處理，支援快速開發和測試
5. **即時分析**: 基於記憶體事件的即時Metrics計算

#### 生產Environment數據流

1. **前端收集**: 同開發Environment的事件收集機制
2. **批次處理**: 同樣的批次處理Policy
3. **Domain Event發布**: 事件透過現有 DDD 系統發布到 MSK
...
#### 1. Observability SDK (`ObservabilityService`)
...
public class AnalyticsController {
    ...
    public ResponseEntity<Void> receiveEvents(
        ...
        try {
            ...
            // 處理事件 (內部會重新設定 MDC，但這裡先設定確保 Controller 層的Logging正確)
...
public class ObservabilityEventPublisher {
    ...
    public void handleAnalyticsEvent(AnalyticsEventReceivedEvent event) {
        if ("kafka".equals(publisherType)) {
            // 生產Environment：發布到 MSK
            publishToKafka(event);
        } else {
            // 開發/測試Environment：記憶體處理
...
```java
// ObservabilityDomain Event (整合現有 DDD 系統)
...
```java
// Observability會話Aggregate Root (整合現有 DDD 架構)
@AggregateRoot(name = "ObservabilitySession", description = "Observability會話Aggregate Root", boundedContext = "Observability", version = "1.0")
...
genai-demo:
  ...
  observability:
    analytics:
      enabled: true
      storage: in-memory  # Metrics存在記憶體中
      retention-minutes: 60
```

#### 生產Environment配置 (MSK Profile)

```yaml
# application-msk.yml (生產Environment啟用)
spring:
  kafka:
...
**現有業務Domain Event Topics (CDK 已定義)**:
...
**新增Observability事件 Topics (需要在 CDK 中添加)**:
...
- 分隔符: 使用 `.` (點) 而非 `-` (破折號)
- Environment變數: 透過 `${spring.profiles.active}` 動態設定

#### 與現有 MDC Tracing系統整合

**現有系統使用 MDC (Mapped Diagnostic Context)**:

- `correlationId`: 主要的Tracing識別碼
- `traceId`: OpenTelemetry Tracing ID  
- `sessionId`: 用戶會話 ID
...
### Environment差異化錯誤處理Policy

#### 1. Profile-Aware 事件處理Resilience
...
#### 2. Environment適應性Circuit Breaker Pattern
...
public class EnvironmentAwareCircuitBreakerService {
    ...
    public void fallbackProcessing(List<UserBehaviorAnalyticsEvent> events, Exception ex) {
        if ("kafka".equals(publisherType)) {
            // 生產Environment：儲存到 MSK 備援 topic
            kafkaBackupService.storeForLaterProcessing(events);
        } else {
            // 開發Environment：儲存到本地檔案或記憶體
...
根據開發標準指導原則，採用以下測試分層Policy：

1. **Unit Test (80%)**: `@ExtendWith(MockitoExtension.class)` - 快速、輕量
2. **切片測試 (15%)**: `@WebMvcTest`, `@DataJpaTest` - 部分 Spring 上下文
3. **End-to-End Test (5%)**: `@SpringBootTest` - 僅關鍵業務流程

### 效能目標

- Unit Test: < 50ms, < 5MB
- Integration Test: < 500ms, < 50MB  
- E2E 測試: < 3s, < 500MB
...
```java
@UnitTest        // 快速Unit Test
@SlowTest        // 標記慢速測試
@IntegrationTest // Integration Test標記
```
...
**決策**: 實施Customer端批次處理，可配置批次大小（50 個事件）和時間窗口（30 秒）。
...
- 即時分析略有延遲（對大多數用例可接受）
- 增加Customer端記憶體使用（影響最小）
- 錯誤處理的額外複雜性

### 2. 整合現有 DDD Domain Event系統

**決策**: 整合現有的 DDD Domain Event系統，而非重新建立獨立的事件處理架構。
...
- 需要適應現有系統的Constraint和模式
- Observability事件需要符合現有的Domain Event規範
- 依賴現有系統的穩定性和效能

### 3. Environment差異化事件處理Policy

**決策**: 根據 Spring Profile 實施不同的事件處理Policy：開發/測試使用記憶體處理，生產使用 MSK。

**理由**:

- 開發Environment快速回饋：記憶體處理提供即時除錯和測試能力
- 生產EnvironmentScalability：MSK 支援大規模分散式事件處理
- 成本效益：開發Environment無需昂貴的 Kafka 基礎設施
- 一致的開發體驗：相同的程式碼在不同Environment中自動適應

**Trade-off**:

- 需要維護兩套事件處理邏輯
- 開發和生產Environment的行為差異可能導致測試盲點
- 配置複雜性增加

### 4. MSK 整合與 DLQ 處理

**決策**: 在生產Environment中使用 Amazon MSK 進行事件處理，並實施完整的 DLQ (Dead Letter Queue) 錯誤處理機制。
...
- 支援大規模、高Availability的事件處理

**Trade-off**:

- AWS 服務依賴和成本考量
- MSK 配置和Monitoring的複雜性
- 需要處理網路分割和服務中斷情況

### Testing

**決策**: 實施漸進式DeploymentPolicy，支援從開發Environment的記憶體處理平滑過渡到生產Environment的 MSK 處理。

**理由**:

- 降低Deployment風險：可以在開發Environment充分測試後再啟用 MSK
...
- 需要維護兩套處理邏輯的Test Coverage
- 配置管理的複雜性
- 開發和生產Environment差異可能導致的潛在問題

## summary

此設計充分整合現有的 DDD Domain Event系統和Environment配置，提供了：

1. **Environment適應性**: 根據 Spring Profile 自動切換事件處理模式
2. **架構一致性**: 重用現有的 DDD 事件基礎設施
3. **生產就緒**: 完整的 MSK 整合和 DLQ 錯誤處理
4. **開發友善**: 記憶體處理支援快速開發和測試
5. **Scalability**: 支援從小規模開發到大規模生產的平滑擴展

這個設計為前端後端Observability整合提供了實用、可維護且符合現有架構的解決方案。
