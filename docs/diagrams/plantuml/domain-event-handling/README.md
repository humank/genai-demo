...
The system adopts a custom domain event handling mechanism based on the Spring framework, with the following main features:

1. **Domain Event Publishing**: Convert domain events to Spring application events through `DomainEventPublisherAdapter`
2. **Event Subscription**: Use custom `@EventSubscriber` annotation to mark event handling methods
3. **Event Routing**: Scan and register event handlers at runtime through `DomainEventSubscriptionManager`
4. **Fallback Mechanism**: Automatically fallback to `DomainEventPublisherService` when `AggregateLifecycle` is unavailable
...
- **Infrastructure Layer**:
  - `DomainEventPublisherAdapter`: Domain event publishing adapter
...
- Trigger points for event publishing
...
Integration tests should be placed in the `/Users/yikaikao/git/genai-demo/app/src/test/java/solid/humank/genaidemo/integration/event/` directory, mainly testing:
...
1. **Event Design**:
   ...
   - Events should contain sufficient context information
...
3. **Testing**:
   ...
   - Verify system state changes after event handling