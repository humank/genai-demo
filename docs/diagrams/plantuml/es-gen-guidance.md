# Event Storming Three-Phase PlantUML Guide

This document provides guidance for using PlantUML to draw Event Storming outputs from three phases, 
including layout, color specifications, element ordering, and best practices.

## Event Storming Overview

Event Storming is a collaborative modeling technique created by Alberto Brandolini for exploring 
complex business domains. It typically consists of three main phases:
...
- Place main processes in the center, branch processes below
...
OrderCancelledEvent --> RequestRefundCommand : If already paid

' Event to read model projections
OrderCreatedEvent --> OrderSummaryReadModel : Projects
OrderItemAddedEvent --> OrderDetailReadModel : Projects
OrderConfirmedEvent --> OrderHistoryReadModel : Projects
InventoryReservedEvent --> InventoryStatusReadModel : Projects
PaymentCompletedEvent --> PaymentStatusReadModel : Projects
DeliveryArrangedEvent --> DeliveryTrackingReadModel : Projects
...
#### Layout Tips
- Use `package` to clearly divide bounded contexts
...
1. **CQRS Pattern Implementation**: In the design level phase, Command Query Responsibility Segregation (CQRS) pattern is typically considered, where read models are the core of the query part.

2. **User Interface Design**: Read models directly influence user interface design and implementation, which needs to be clearly defined in the design level phase.

3. **Performance Optimization**: The design level phase needs to consider read model performance optimization strategies, such as caching and indexing.
...
5. **Data Consistency**: Read models help solve eventual consistency issues, which need to be considered in the design level phase regarding event ordering and data synchronization.
...
7. **Cross-Bounded Context Data Integration**: Read models can integrate data from multiple bounded contexts, providing users with a unified view.
...
OrderCreatedEvent --> OrderSummaryReadModel : Projects
OrderItemAddedEvent --> OrderDetailReadModel : Projects
PaymentCompletedEvent --> OrderSummaryReadModel : Updates payment status
```

2. **Define Read Model Update Strategies**:
...
3. **Read Model Version Management**: Consider read model evolution strategies, especially in long-running systems.

4. **Read Model Caching Strategy**: Design appropriate caching mechanisms, including cache invalidation strategies and update mechanisms.

5. **Cross-Bounded Context Read Models**: Clearly define read models that span multiple bounded contexts and design their data synchronization mechanisms.

### Representing Read Models in PlantUML

In design level phase PlantUML diagrams, read models should be represented in the following ways:

1. **Include relevant read models in each bounded context**:
...
3. **Represent read model relationships with user interfaces**:

```puml
rectangle "User" as User #FFD700
User --> OrderSummaryReadModel : Queries
OrderSummaryReadModel --> User : Displays
```

4. **Represent read model update strategies**:
...
OrderConfirmedEvent --> CustomerOrderHistoryReadModel : Projects
PaymentCompletedEvent --> CustomerOrderHistoryReadModel : Projects
DeliveryCompletedEvent --> CustomerOrderHistoryReadModel : Projects
```
...
1. **Big Picture Exploration Phase**: Focus on "what happened", capturing main events and issues
...
Each phase's diagrams have their specific focus points and target audiences. Through gradual deepening and refinement, teams can transition from high-level business overview to detailed technical design, ensuring software implementation remains aligned with business requirements.