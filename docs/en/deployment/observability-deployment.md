...
if [ "$RDS_STATUS" != "available" ]; then
    echo "❌ RDS Repository狀態異常: $RDS_STATUS"
    exit 1
fi
echo "✅ RDS Repository狀態正常"
...
if [ "$METRICS_RESPONSE" != "200" ]; then
    echo "❌ Metrics端點異常: HTTP $METRICS_RESPONSE"
    exit 1
fi
echo "✅ Metrics端點正常"

# 4. 檢查Monitoring和Alerting
echo "4. 檢查Monitoring配置..."
...
if [ "$METRIC_COUNT" -lt "10" ]; then
    echo "⚠️  CloudWatch Metrics數量較少: $METRIC_COUNT"
else
    echo "✅ CloudWatch Metrics正常: $METRIC_COUNT 個Metrics"
fi
...
if [ "$ALARM_COUNT" -lt "5" ]; then
    echo "⚠️  CloudWatch Alerting數量較少: $ALARM_COUNT"
else
    echo "✅ CloudWatch Alerting配置正常: $ALARM_COUNT 個Alerting"
fi
...
# 2. 執行回滾
echo "2. 執行 Kubernetes Deployment回滾..."
...
if [ "$HEALTH_STATUS" != "200" ]; then
    echo "❌ 回滾後Health Check失敗: HTTP $HEALTH_STATUS"
    echo "請檢查應用程式Logging並考慮進一步的回滾操作"
    exit 1
fi
...
if [ "$RDS_ENCRYPTED" != "True" ]; then
    echo "❌ RDS Repository未啟用加密"
else
    echo "✅ RDS Repository加密已啟用"
fi
...
if [ "$CLOUDTRAIL_STATUS" != "True" ]; then
    echo "❌ CloudTrail Logging未啟用"
else
    echo "✅ CloudTrail Logging已啟用"
fi
