...
| Command (Commands) | #1E90FF (蓝色) | 触发事件的行为或意图 |
| Aggregate (Aggregates) | #FFFF00 (黄色) | 确保业务规则的实体 |
| Policy/政策 (Policies) | #800080 (紫色) | 当事件发生时应执行的业务规则 |
| 读模型 (Read Models) | #32CD32 (绿色) | 用户查看的信息或视图 |
| 外部系统 (External Systems) | #FF69B4 (粉色) | 与核心系统交互的外部系统 |
| 参与者 (Actors) | #FFD700 (金色) | 执行Command的人或系统 |
| 热点/问题 (Hotspots/Issues) | #FF0000 (红色) | 需要解决的问题或决策点 |
| 服务 (Services) | #ADD8E6 (浅蓝色) | 协调Aggregate和处理业务逻辑 |
...
title 訂單系統Event Storming圖 - 流程建模階段
...
' Policy
rectangle "庫存預留Policy" as InventoryReservationPolicy #800080
rectangle "支付處理Policy" as PaymentProcessingPolicy #800080
rectangle "訂單取消Policy" as OrderCancellationPolicy #800080
...
title 訂單系統Event Storming圖 - 設計級別階段

' 使用包来表示Bounded Context
package "訂單上下文" {
    ' Aggregate
    rectangle "訂單" as OrderAggregate #FFFF00
...
package "配送上下文" {
    ...
    rectangle "配送Tracing" as DeliveryTrackingReadModel #32CD32
...
#### 布局技巧
- 使用 `package` 清晰划分Bounded Context
- 在每个上下文内部保持一致的元素排列
- 使用标注说明上下文之间的集成点
- 突出显示与外部系统的交互
- 将相关的上下文放在一起
...
1. **Command Query Responsibility Segregation (Command Query Responsibility Segregation (CQRS))模式实现**：在设计级别阶段，通常会考虑实现Command查询责任分离(Command Query Responsibility Segregation (Command Query Responsibility Segregation (CQRS)))模式，其中读模型是查询部分的核心。

2. **用户界面设计**：读模型直接影响用户界面的设计和实现，在设计级别阶段需要明确定义。

3. **Performance优化**：设计级别阶段需要考虑读模型的Performance优化Policy，如缓存、索引等。
...
7. **跨Bounded Context数据整合**：读模型可以整合来自多个Bounded Context的数据，为用户提供统一视图。
...
3. **读模型的版本管理**：考虑读模型的版本演进Policy，特别是在长期运行的系统中。

4. **读模型的缓存Policy**：设计适当的缓存机制，包括缓存失效Policy和更新机制。

5. **跨Bounded Context的读模型**：明确定义跨多个Bounded Context的读模型，并设计其数据同步机制。

### 读模型在 PlantUML 中的表示

在设计级别阶段的 PlantUML 图中，应该采用以下方式表示读模型：

1. **在各个Bounded Context中包含相关读模型**：
...
这样可以更全面地展示系统的设计，包括Command端和查询端的完整视图，以及读模型如何支持用户交互和业务决策。
...
### 生成图表
使用以下Command将 PlantUML 文件转换为 SVG 格式：
...
2. **流程建模阶段**：增加"谁做了什么"和"为什么"，添加Command、Aggregate和Policy
3. **设计级别阶段**：增加"如何实现"，划分Bounded Context和服务
