...
# Security PerspectiveCross-Viewpoint Application

## Overview

Security Perspective是一個橫切Concern，影響系統架構的所有視點。本文件詳細說明Security考量如何在各個Architectural Viewpoint中體現和實現。

## 跨視點Security應用

### Functional Viewpoint中的Security考量
...
public class Customer {
    ...
    public CustomerSummary getSummary(SecurityContext securityContext) {
        if (securityContext.isOwner(this.id) || securityContext.hasRole("ADMIN")) {
            ...
            throw new UnauthorizedAccessException("無權限查看Customer資訊");
        }
    }
}
```
...
public class PaymentProcessingService {
    ...
    private void validatePaymentPermissions(ProcessPaymentCommand command, SecurityContext context) {
        ...
        if (!context.canProcessPaymentForCustomer(command.getCustomerId())) {
            throw new InsufficientPermissionException("無權限處理此Customer的支付");
        }
        
        // 檢查支付金額限制
        Money userLimit = context.getPaymentLimit();
        if (command.getAmount().isGreaterThan(userLimit)) {
            throw new PaymentLimitExceededException("支付金額超過User限制");
        }
    }
}
```
...
### Information Viewpoint中的Security考量
...
### Concurrency Viewpoint中的Security考量
...
### Development Viewpoint中的Security考量
...
### Operational Viewpoint中的Security考量
...
### Functional Viewpoint安全檢查

- [ ] 所有敏感業務操作都有適當的授權檢查
- [ ] 業務規則包含安全驗證邏輯
- [ ] Domain Event包含安全審計資訊
- [ ] Aggregate Root保護敏感資料存取

### Information Viewpoint安全檢查

- [ ] 敏感資料在Repository中加密存儲
- [ ] 資料存取有適當的權限控制
- [ ] Domain Event包含安全分類
- [ ] 查詢結果根據權限進行資料遮罩

### Concurrency Viewpoint安全檢查
