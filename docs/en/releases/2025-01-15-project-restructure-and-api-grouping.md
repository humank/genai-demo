...
# 專案Refactoring和 API 分組優化 - 2025-01-15

## 📋 變更概述

本次更新主要針對專案結構重整和 API 分組Policy優化，提升專案的Maintainability和 API 文檔的使用體驗。
...
│   ├── verify-deployment.sh # Deployment
...
- **Deployment相關**: `deploy-to-eks.sh`, `k8s/`, `aws-eks-architecture.md` → `deployment/`
...
- `customer-api`: Customer端 API (面向終端Customer)
- `operator-api`: 運營管理 API (面向平台運營者)
- `system-api`: 系統管理 API (面向System Administrator)

#### 📊 分組詳細說明

##### Customer端 API (`customer-api`)

**目標User**: 終端Customer (Customer)
**包含路徑**:
...
- `/api/delivery-tracking/**` - 配送Tracing

##### 運營管理 API (`operator-api`)

**目標User**: 平台運營者 (Operator/Admin)
**包含路徑**:

- `/api/customers/**` - Customer管理
- `/api/orders/**` - 全平台訂單管理
- `/api/products/**` - 商品管理 (CRUD)
- `/api/inventory/**` - 庫存管理
- `/api/pricing/**` - 定價Policy
- `/api/payments/**` - 支付管理
...
**目標User**: System Administrator、DevOps
**包含路徑**:
...
- **購物車Aggregate**: 實作完整的購物車業務邏輯和優惠計算
- **CustomerAggregate**: 增強Customer模型，支援紅利點數和通知偏好
- **訂單Aggregate**: 完善訂單生命週期管理和狀態轉換

#### Testing

- **BDD 測試**: 實作消費者購物流程的行為驅動測試
- **Architecture Test**: 使用 ArchUnit 確保 DDD 架構合規性
- **Integration Test**: 完整的 API 端點測試覆蓋
- **Unit Test**: 領域邏輯和業務規則的Unit Test
...
- **多階段構建**: 最小化最終映像大小
- **JVM 調優**: 容器Environment專用的 JVM 參數
- **Health Check**: 完整的應用程式Monitoring機制
...
- **清晰的目錄結構**: Developer能快速找到相關檔案
- **功能分組**: 相關功能集中管理，降低維護成本
- **文檔完整性**: 每個目錄都有詳細的使用說明

### API 使用體驗改善

- **角色導向分組**: 不同User只看到相關的 API
- **中文標籤**: 更直觀的 API 分類和描述
- **智能路徑匹配**: 自動將 API 歸類到正確的分組

### 架構品質提升

- **DDD 合規性**: 通過Architecture Test確保Design Principle
- **測試覆蓋**: 多層次的測試Policy保證Code Quality
- **Containerization優化**: ARM64 原生支援和效能調優
...
- [Docker Deployment指南](../DOCKER_GUIDE.md)
- [API 版本管理Policy](../api/API_VERSIONING_STRATEGY.md)
