# Troubleshooting Guide

> **Status**: ✅ Active  
> **Last Updated**: 2024-11-19

## Overview

This section contains troubleshooting guides for common issues.

## Contents

### Application Issues
- [Application Debugging](application-debugging.md) - Debug application issues
- [Performance Degradation](performance-degradation.md) - Performance troubleshooting
- [Common Issues](common-issues.md) - Common problems and solutions

### Infrastructure Issues
- [Database Issues](database-issues.md) - Database troubleshooting
- [Kubernetes Issues](kubernetes-issues.md) - K8s troubleshooting
- [Network Connectivity](network-connectivity.md) - Network issues
- [Distributed System Issues](distributed-system-issues.md) - Distributed system problems

### Debugging Tools
- [Debugging Guide](debugging-guide.md) - Debugging tools and techniques

## Quick Reference

For operational procedures, see:
- [Operational Viewpoint](../../viewpoints/operational/README.md)
- [Runbooks](../runbooks/README.md)

## Related Documentation

- [Operations Guide](../README.md)
- [Monitoring Guide](../monitoring/README.md)
- [Database Operations Examples](../../examples/database-operations-examples.md)

---

**Note**: Many troubleshooting guides are already available in this directory.
