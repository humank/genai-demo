...
Using Domain-Driven Design (DDD) methodology, the promotion module is designed as an independent subdomain:
...
2. **Architecture Issue Fixes**:
   - Reclassified `Voucher` from value object to entity and moved from `valueobject` package to `entity` package.
