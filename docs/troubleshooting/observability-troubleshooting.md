...
This guide provides common issue diagnosis and solutions for the frontend-backend observability integration system, including connection issues, performance problems, and data processing issues.
...
# 2. Check Kafka connection (production)
...
timeout 5 wscat -c ws://localhost:8080/ws/analytics?sessionId=health-check 2>/dev/null && echo "✅ WebSocket normal" || echo "❌ WebSocket connection failed"
...
- No event reception logs in backend

**Diagnosis Steps**:

```javascript
// Execute diagnosis in browser console
console.log('=== Frontend Observability Diagnosis ===');
...
1. **Check event conversion logic**:
...
**Note: WebSocket functionality is not fully implemented yet, will be developed in the next phase**
...
- Event processing delay exceeds 1 second from send to process
...
public class IdempotentEventProcessor {
    ...
    @Scheduled(fixedRate = 600000) // Clean every 10 minutes
    public void cleanupProcessedEvents() {
        // Clean event IDs from 1 hour ago
        // Actual implementation might need Redis or database
    }
...
- [Operational Overview](../viewpoints/operational/observability-overview.md)
