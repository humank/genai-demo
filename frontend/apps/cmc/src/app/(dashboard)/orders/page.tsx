...
import { useOrderStatusStats, useStats } from '@repo/api-client/hooks'
import {
  Activity,
  CheckCircle,
  Clock,
...
export default function OrdersPage() {
  ...
  const orderStats = React.useMemo(() => {
    ...
    const statusMap = Object.fromEntries(
      (orderStatusData || []).map(s => [s.status, s.count])
    )
    const completedOrders = statusMap['COMPLETED'] || 0
    const pendingOrders = (statusMap['PENDING'] || 0) + (statusMap['CREATED'] || 0)
