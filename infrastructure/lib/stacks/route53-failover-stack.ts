...
export class Route53FailoverStack extends cdk.Stack {
    ...
    private createHealthCheck(
        ...
        const healthCheck = new route53.CfnHealthCheck(this, `${regionType}HealthCheck`, {
            ...
            measureLatency: true,
            enableSni: true,
            // Add missing properties for proper API configuration
            searchString: '"status":"UP"',  // Look for this string in the response
            insufficientDataHealthStatus: 'Failure',  // Treat insufficient data as failure
            inverted: false,  // Don't invert the health check result
            disabled: false,  // Enable the health check
            // Add alarm identifier for CloudWatch integration
            alarmIdentifier: {
                name: `${projectName}-${environment}-${regionType.toLowerCase()}-health-alarm`,
                region: this.region
            },
            tags: [
                {
