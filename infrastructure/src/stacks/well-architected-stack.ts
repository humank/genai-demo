...
/**
 * AWS Well-Architected Tool Stack
 * 
 * Implements automated architecture assessment based on 6 pillars:
 ...
 * 
 * Features:
 * - Automated workload creation and assessment
 * - Periodic review scheduling
 * - Risk prioritization and improvement recommendations
 * - Automated action plan generation
 * - Integration with SNS for notifications
 */
export class WellArchitectedStack extends cdk.Stack {
  public readonly workload: cdk.CfnResource;
  public readonly assessmentLambda: lambda.Function;
  public readonly reportBucket: s3.Bucket;

  constructor(scope: Construct, id: string, props: WellArchitectedStackProps) {
    super(scope, id, props);

    // Create S3 bucket for Well-Architected reports
    this.reportBucket = new s3.Bucket(this, 'WellArchitectedReports', {
      bucketName: `well-architected-reports-${props.environment}-${this.account}`,
    ...
    // Create SNS topic for Well-Architected notifications
    const notificationTopic = new sns.Topic(this, 'WellArchitectedNotifications', {
      displayName: 'Well-Architected Tool Notifications',
      topicName: `well-architected-notifications-${props.environment}`,
    });
    ...
    this.workload = new cdk.CfnResource(this, 'Workload', {
      ...
      properties: {
        ...
        Description: `GenAI Demo Platform - ${props.environment} Environment`,
        Environment: props.environment.toUpperCase(),
        ReviewOwner: props.reviewOwner,
        
        // Architecture details
        ArchitecturalDesign: 'https://github.com/your-org/genai-demo/docs/architecture',
        
        // AWS Regions
        AwsRegions: ['ap-northeast-1', 'ap-northeast-1'], // Taiwan and Japan
        
        // Account IDs
        AccountIds: [this.account],
        
        // Industry and industry type
        Industry: 'Technology',
        ...
        // Pillar priorities (1 = highest priority)
        PillarPriorities: [
          'security', // Priority 1
          'reliability', // Priority 2
          'operationalExcellence', // Priority 3
          'performance', // Priority 4
          'costOptimization', // Priority 5
    ...
    // Create IAM role for assessment Lambda
    const assessmentRole = new iam.Role(this, 'AssessmentLambdaRole', {
      assumedBy: new iam.ServicePrincipal('lambda.amazonaws.com'),
      description: 'Role for Well-Architected assessment automation',
      managedPolicies: [
        iam.ManagedPolicy.fromAwsManagedPolicyName('service-role/AWSLambdaBasicExecutionRole'),
      ],
    });

    // Add Well-Architected permissions
    assessmentRole.addToPolicy(new iam.PolicyStatement({
    ...
    // Add S3 permissions for report storage
    this.reportBucket.grantReadWrite(assessmentRole);

    // Add SNS permissions for notifications
    notificationTopic.grantPublish(assessmentRole);

    // Create Lambda function for automated assessment
    this.assessmentLambda = new lambda.Function(this, 'AssessmentFunction', {
      runtime: lambda.Runtime.PYTHON_3_11,
      handler: 'index.handler',
      code: lambda.Code.fromInline(this.getAssessmentLambdaCode()),
      role: assessmentRole,
      timeout: cdk.Duration.minutes(15),
      memorySize: 512,
      environment: {
        WORKLOAD_ID: this.workload.getAtt('WorkloadId').toString(),
    ...
    // Create EventBridge rule for weekly assessment
    const weeklyAssessmentRule = new events.Rule(this, 'WeeklyAssessment', {
    ...
    weeklyAssessmentRule.addTarget(new targets.LambdaFunction(this.assessmentLambda));

    // Create EventBridge rule for milestone creation (monthly)
    const monthlyMilestoneRule = new events.Rule(this, 'MonthlyMilestone', {
      schedule: events.Schedule.cron({
        day: '1',
        hour: '10',
        minute: '0',
    ...
    monthlyMilestoneRule.addTarget(new targets.LambdaFunction(this.assessmentLambda, {
    ...
    new cdk.CfnOutput(this, 'WorkloadId', {
      value: this.workload.getAtt('WorkloadId').toString(),
      description: 'Well-Architected Workload ID',
      exportName: `${props.environment}-workload-id`,
    });

    new cdk.CfnOutput(this, 'WorkloadArn', {
      value: this.workload.getAtt('WorkloadArn').toString(),
      description: 'Well-Architected Workload ARN',
    });

    new cdk.CfnOutput(this, 'ReportBucket', {
      value: this.reportBucket.bucketName,
      description: 'S3 bucket for Well-Architected reports',
    });

    new cdk.CfnOutput(this, 'AssessmentLambdaArn', {
      value: this.assessmentLambda.functionArn,
  ...
  private getAssessmentLambdaCode(): string {
    return `
import json
import boto3
import os
