...
2. **hexagonal-architecture-overview.puml** - Hexagonal Architecture概覽
3. **application-services-overview.puml** - 應用服務概覽
4. **infrastructure-layer-overview.puml** - Infrastructure Layer概覽
5. **bounded-contexts-overview.puml** - 有界上下文概覽
6. **domain-events-flow.puml** - Domain Event流程
...
2. **event-storming-process-level.puml** - Event Storming Process Level
...
### 2. 圖表品質
- **標準化配色**：Event Storming 使用標準顏色（橙色事件、黃色Actor、藍色Command）
- **多層次視圖**：從概覽到詳細設計的完整層次
- **關係映射**：自動識別Aggregate Root、Entity、Value Object之間的關係

### 3. 架構洞察
- **Hexagonal Architecture**：清晰展示領域核心、Application Layer、Infrastructure Layer和Interface Layer
- **DDD 模式**：完整識別Aggregate Root、Entity、Value Object和Domain Event
- **事件驅動**：豐富的Domain Event支援業務流程自動化
...
### 2. 架構可視化
- 清晰展示系統架構
- 識別Design Pattern和Best Practice
- 支援架構決策和Refactoring
